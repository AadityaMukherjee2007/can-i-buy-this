...
async def get_link_token(user: User = Depends(get_current_user)):
    ...
    return LinkTokenResponse(link_token=result["link_token"])
...
async def exchange_token(
    ...
    result = await exchange_public_token(payload.public_token)
    biz = user.business
    biz.plaid_access_token = result["access_token"]
    biz.plaid_item_id = result["item_id"]
    await db.commit()
    return {"status": "success", "item_id": result["item_id"]}
